...
public class testInputController : MonoBehaviour {
	...
	private Dictionary<string,int> map; // must change to int,int for pianokeyboardmap or str,int for computerkeyboard

	// Use this for initialization
	void Start () {
		map = gameObject.GetComponent<testInputMap>().initializeComputerKeyboardMap();
		//map = gameObject.GetComponent<InputMap>().initializePianoKeyboardMap();
	}

	// sets pressed to true or false depending on if key is pressed
	void computerKeyboard() {
		foreach (KeyValuePair<string, int> entry in map) {
			...
			if (Input.GetKeyDown(entry.Key)) {
				...
				keyObj = gameObject.GetComponent<testKeyGenerator>().keyArray[entry.Value-1];
